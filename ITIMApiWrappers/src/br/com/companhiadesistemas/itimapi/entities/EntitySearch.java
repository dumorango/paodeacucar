...
public class EntitySearch {
	...
	public static Collection<AccountEntity> searchAccountByFilter(String filter) throws ObjectNotFoundException, ModelCommunicationException, PartialResultsException{
		AccountSearch as = new AccountSearch();
		SearchParameters searchParams = new SearchParameters();
		searchParams.setScope(SearchParameters.ONELEVEL_SCOPE );
		SearchResults sr; 
		sr = as.searchByFilter(new CompoundDN(new DistinguishedName("ou=GPA,DC=TIM")),
					filter, 
					searchParams);
	...
	public static Collection<AccountEntity> getAccountsByServiceName(String serviceName) throws PartialResultsException, ObjectNotFoundException, ModelCommunicationException{
	...
	public static Collection<AccountEntity> getAccountsByServiceNameAndOwner(String serviceName,PersonEntity owner) throws PartialResultsException, ObjectNotFoundException, ModelCommunicationException{
		Collection<ServiceEntity> services = searchServiceByFilter("(erServiceName="+serviceName+")");
		if(services.isEmpty()) return null;
		String serviceDN = ((ServiceEntity) services.iterator().next()).getDistinguishedName().toString();
